...
fn api_builder(config: &Config) -> reqwest::ClientBuilder {
    ...
    headers.insert(
        "PRIVATE-TOKEN",
        HeaderValue::from_str(config.private_token.as_ref().unwrap()).unwrap(),
    );
...
pub fn get_grp_jobs(config: &Config, scope: JobScope) -> Vec<String> {
    let uri = format!(
        "/api/v4/projects/{}/jobs?pagination=keyset&per_page=100&order_by=id&sort=asc",
        config.group_id.unwrap()
    );
...
mod test_http {
    ...
    async fn test_api_get() {
        ...
        let response = async {
            api_get(&config, "api/v4/projects")
            .send()
            .await
            .unwrap()
            .text()
        ...
        // let text = block_on(response).unwrap();
        // println!("{:?}", text);
