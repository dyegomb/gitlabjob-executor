...
class Testes(unittest.TestCase):
    ...
    def teste9_repositories(self):
        """Teste de procedimentos em repositórios"""

        repos = self.gitjob.get_repositories()
        self.assertIsInstance(repos, dict, "Lista de repositórios de projeto \
não obtida")
        
        grp_id = environ.get("GROUP_ID")
        repos = self.gitjob.get_repositories(groupid=grp_id)
        self.assertIsInstance(repos, dict, "Lista de repositórios de grupo \
não obtida")

        # Excluir imagens de repositórios, aguardar atualização
        #for prjid, repoids in repos.items():
        #    for repid in repoids:
        #        resp = self.gitjob.delete_bulkrep(repo_id=repid, 
        #                                          proj_id=prjid)
        #        self.assertEqual(resp, 202, "Erro ao excluir tag de imagens")
